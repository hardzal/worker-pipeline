...
import { currentPrismaTimestamp, type PrismaDb } from "../../config/prisma.js";
...
export function createJobRepository(
  ...
  return {
    ...
    async findMany(params: FindManyJobsParams) {
      const { limit, cursor } = params;

      let query = prisma.orm.public.Job.orderBy([
      ...
      if (cursor) {
        query = query.cursor({
          createdAt: Temporal.PlainDateTime.from(cursor.createdAt),
          id: cursor.id,
        });
      }

      const jobs = await query.limit(limit).all();
    ...
    async getJob(jobId: string) {
      ...
      const steps = await prisma.orm.public.JobStep.where({ jobId })
